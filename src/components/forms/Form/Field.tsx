...
export function Field<T extends FieldTypes>(props: JengaFieldProps<T>) {
  const allProps: JengaFullFieldProps<T> = useFormProps(props);
  ...
  inputType = inputType ?? (child?.type as any)?.jengaInputType ?? 'Text';

  const __props = useField<T, JengaFullFieldProps<T>>(allProps, {
